...
class AICommandProcessor(private val context: Context) {
    ...
    private fun stripRootLevelSystemManagedFields(command: DataCommand): DataCommand {
        ...
        //
        // IMPORTANT: 'id' is handled specially:
        // - For CREATE_DATA: strip 'id' (will be generated by service)
        // - For UPDATE_DATA: keep 'id' (identifies which entry to update)
        val rootSystemManagedFields = setOf("created_at", "updated_at", "schema_id", "tooltype")
        ...
        val cleanedEntries = entries.map { entry ->
            if (entry is Map<*, *>) {
                ...
                // Strip systemManaged fields (always)
                ...
                // Strip 'id' for CREATE_DATA only (generated by service)
                // For UPDATE_DATA, keep 'id' to identify the entry to update
                if (command.type == "CREATE_DATA" && mutableEntry.containsKey("id")) {
                    mutableEntry.remove("id")
                    LogManager.aiService("Stripped 'id' for CREATE_DATA (will be generated)", "DEBUG")
                }
