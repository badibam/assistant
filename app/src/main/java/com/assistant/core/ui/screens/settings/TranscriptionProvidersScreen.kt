...
/**
 * Transcription Providers configuration screen
 *
 * Displays list of available transcription providers with:
 ...
 *
 * Pattern mirrors AIProvidersScreen for consistency
 */
@OptIn(ExperimentalFoundationApi::class)
@Composable
fun TranscriptionProvidersScreen(
    onBack: () -> Unit
) {
    ...
    var providers by remember { mutableStateOf<List<TranscriptionProviderInfo>>(emptyList()) }
    ...
    LaunchedEffect(Unit) {
        coordinator.executeWithLoading(
            operation = "transcription_provider_config.list",
            ...
            if (!hasActiveProvider && firstConfigured != null) {
                coordinator.processUserAction(
                    "transcription_provider_config.set_active",
    ...
    configuringProviderId?.let { providerId ->
        val registry = remember { TranscriptionProviderRegistry(context) }
        ...
        provider?.let {
            ...
            LaunchedEffect(providerId) {
                coordinator.executeWithLoading(
                    operation = "transcription_provider_config.get",
            ...
            if (!isLoadingConfig) {
                provider.getConfigScreen(
                    ...
                    onSave = { configJson ->
                        coroutineScope.launch {
                            val result = coordinator.processUserAction(
                                "transcription_provider_config.set",
                                mapOf(
                    ...
                    onReset = {
                        ...
                        coroutineScope.launch {
                            val result = coordinator.processUserAction(
                                "transcription_provider_config.delete",
    ...
    Column(
        ...
        // Header
        UI.PageHeader(
            title = s.shared("settings_transcription"),
            subtitle = null,
            icon = null,
